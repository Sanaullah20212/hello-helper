<?php get_header(); ?>

<!-- Category Header -->
<section class="premium-banner" style="padding: 1.5rem 0;">
    <div class="page-container">
        <div class="premium-content">
            <h1 class="premium-text" style="margin-bottom: 0;">
                <?php 
                if (is_category()) {
                    single_cat_title();
                } elseif (is_tag()) {
                    single_tag_title();
                } elseif (is_author()) {
                    the_author();
                } else {
                    the_archive_title();
                }
                ?>
            </h1>
            <?php if (category_description()) : ?>
                <p style="color: var(--muted-foreground); font-size: 0.875rem; margin-top: 0.5rem;">
                    <?php echo category_description(); ?>
                </p>
            <?php endif; ?>
        </div>
    </div>
</section>

<!-- Category Badges -->
<section class="category-badges">
    <div class="page-container">
        <div class="badges-wrapper">
            <?php
            $categories = get_categories(array(
                'orderby' => 'count',
                'order' => 'DESC',
                'number' => 10,
                'hide_empty' => true,
            ));
            
            $current_cat = get_queried_object();
            
            foreach ($categories as $category) :
                $is_active = (is_category() && $current_cat->term_id == $category->term_id) ? ' active' : '';
            ?>
                <a href="<?php echo get_category_link($category->term_id); ?>" class="category-badge<?php echo $is_active; ?>">
                    <?php echo esc_html($category->name); ?>
                </a>
            <?php endforeach; ?>
        </div>
    </div>
</section>

<!-- Movie Grid -->
<section class="movie-grid">
    <div class="page-container">
        <div class="section-header">
            <?php echo btspro24_icon('film', 'w-5 h-5'); ?>
            <h2 class="section-title">
                <?php 
                if (is_category()) {
                    single_cat_title();
                } else {
                    echo 'Posts';
                }
                ?>
            </h2>
            <span class="section-count">(<?php echo $wp_query->found_posts; ?> টি)</span>
        </div>
        
        <?php if (have_posts()) : ?>
            <div class="movies-wrapper">
                <?php while (have_posts()) : the_post(); ?>
                    <a href="<?php the_permalink(); ?>" class="movie-card">
                        <img src="<?php echo btspro24_get_poster(); ?>" alt="<?php the_title_attribute(); ?>" loading="lazy">
                        <div class="movie-card-overlay">
                            <?php 
                            $categories = get_the_category();
                            if (!empty($categories)) : 
                            ?>
                                <span class="movie-card-category"><?php echo esc_html($categories[0]->name); ?></span>
                            <?php endif; ?>
                            
                            <div class="movie-play-btn">
                                <?php echo btspro24_icon('play'); ?>
                            </div>
                            
                            <h3 class="movie-card-title"><?php the_title(); ?></h3>
                            <div class="movie-card-date">
                                <?php echo btspro24_icon('calendar'); ?>
                                <span><?php echo get_the_date('Y'); ?></span>
                            </div>
                        </div>
                    </a>
                <?php endwhile; ?>
            </div>
            
            <?php btspro24_pagination(); ?>
            
        <?php else : ?>
            <div class="empty-state">
                <?php echo btspro24_icon('film', 'w-12 h-12'); ?>
                <p>কোনো কন্টেন্ট পাওয়া যায়নি।</p>
            </div>
        <?php endif; ?>
    </div>
</section>

<?php get_footer(); ?>
